...
// ============================================================================
// TEST: AudioEncoder → AAC Files (without Muxer)
// ============================================================================
// Ce script teste AudioEncoder pour générer des fichiers .aac directement
// - Capture depuis WASAPI
// - Feed à AudioEngine (mix)
// - AudioEncoder encode PCM → AAC
// - Écrit les packets AAC directement dans des fichiers .aac (sans MP4 muxer)
// ============================================================================

console.log('🔍 Testing AudioEncoder → AAC files (without Muxer)');
console.log('   This tests AAC encoding and writes packets directly to .aac files');
console.log('   Recording for 10 seconds...\n');

// Output files
const outputDir = __dirname;
const bothOutputPath = path.join(outputDir, 'test_audioencoder_both.aac');
const desktopOutputPath = path.join(outputDir, 'test_audioencoder_desktop.aac');
const micOutputPath = path.join(outputDir, 'test_audioencoder_mic.aac');
...
// ============================================================================
// AudioEngine Setup with Custom Encoder
// ============================================================================

console.log(`📝 AAC files will be created:`);
...
// Initialize with raw AAC mode (third parameter = true)
if (!engineDesktop.initialize(desktopOutputPath, 192000, true)) {
  console.error('❌ Failed to initialize desktop AudioEngineEncoder');
  process.exit(1);
}

if (!engineMic.initialize(micOutputPath, 192000, true)) {
  console.error('❌ Failed to initialize mic AudioEngineEncoder');
  process.exit(1);
}

if (!engineBoth.initialize(bothOutputPath, 192000, true)) {
...
setTimeout(() => {
  ...
  console.log('\n🎉 Test complete!');
  console.log('   Check the AAC files for artefacts:');
